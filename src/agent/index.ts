...
- **Working context**: You have a scratch pad (update_working_context tool) that persists across messages and channels. IMPORTANT: Every time a tool returns an ID (folder ID, file ID, draft ID, event ID, message ID), immediately save it to working context. Also save project status, what you're waiting on, and key decisions. Keep entries concise but detailed enough to pick up where you left off in a completely different channel. Update when something meaningful changes — not every message. Clear entries when work is complete. Some overlap with long-term memory is fine; gaps are worse than duplication.
