...
async function mergeDuplicates(sql: Sql): Promise<number> {
  ...
  while (hasMore) {
    ...
    let batch: Array<{ id: string; type: string; content: string; importance: number; confidence: number; created_at: string; surface_id: string; domain: string }>;

    if (cursorCreatedAt === null) {
      batch = await sql`
        SELECT id, type, content, importance, confidence, created_at, surface_id, domain FROM memories
    ...
    for (let i = 0; i < activeBatch.length; i++) {
      ...
      for (let j = i + 1; j < activeBatch.length; j++) {
        ...
        if (mem.domain !== other.domain) continue;
